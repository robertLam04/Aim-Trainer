...
void BasicSpawner::init(int nStartingEntities) {
    ...
    while (targets.size() < nStartingEntities) {
        ...
        auto candidate = std::make_unique<Entities::CircleTarget>(circle, position, velocity);
...
void BasicSpawner::deleteEntity(std::vector<std::unique_ptr<Entities::Entity>>::iterator target) {
